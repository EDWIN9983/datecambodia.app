...
export default function EditProfilePage() {
    ...
    function deletePhoto(url: string) {
        setPhotos((prev) => {
            if (prev.length <= 1) {
                setError("You must keep at least one profile photo");
                return prev;
            }
    ...
    return (
        <PageShell title="Edit Profile">
            <div className="mx-auto w-full max-w-md app-card rounded-xl p-6 shadow space-y-5">
                    ...
                    {photos.map((p) => (
                        <div key={p} className="relative group">
                            ...
                            <button
                                ...
                                className="absolute top-1 right-1 text-xs bg-red-600/80 text-white rounded-full px-2 py-0.5 opacity-90 hover:opacity-100"
                                title="Remove photo"
                            >
                                ✕
                            </button>
                ...
                <label className="cursor-pointer inline-block app-primary glow-btn text-center py-2 px-4 rounded font-medium">
                    Add photo
                    <input
                        type="file"
                        accept="image/*"
                ...
                <button
                    onClick={save}
                    disabled={loading}
                    className="w-full app-primary glow-btn py-3 rounded-lg font-semibold disabled:opacity-50"
                >
